...
public function getFilteredTransactions($filters, $limit, $offset)
{
    ...
    if (!empty($filters['date_to'])) {
        $builder->where('transaksi.tanggal_transaksi <=', $filters['date_to']);
    }
    if (!empty($filters['tipe_transaksi'])) {
        $builder->where('transaksi.tipe_transaksi', $filters['tipe_transaksi']); // Fixed ENUM issue
    }
    if (!empty($filters['nama_peminta'])) {
        $builder->where('transaksi.nama_peminta', $filters['nama_peminta']);
    }
...
public function countFilteredTransactions($filters)
{
    ...
    if (!empty($filters['tipe_transaksi'])) {
        $builder->where('tipe_transaksi', $filters['tipe_transaksi']); // Fixed ENUM issue
    }
    if (!empty($filters['nama_peminta'])) {
        $builder->where('nama_peminta', $filters['nama_peminta']);
    }
